...
Firstly, the meanexpression in gene_data were ordered in the descending order using order function and then it was saved in gene_data_sorted file. After that, first 10 rows containing 10 genes with highest meanexpression were displayed in gene_data_sorted using head function. 

```{r}
gene_data_sorted <- gene_data[order(-gene_data$meanexpression), ] # order(-gene_data$meanofcolumns) sors the meanexpression in descending order
head(gene_data_sorted, 10)
```
...
```{r}
# creating logical vectors for genes with meanexpression <10
...
data("gene_data")
hist(gene_data$meanexpression,xlab="mean expression", ylab="number of genes", main="Mean value of gene expression in gene_data")
